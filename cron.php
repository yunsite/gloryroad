...
*
* @package phpBB3
* @version $Id: cron.php,v 1.13 2006/06/13 15:06:31 acydburn Exp $
...
switch ($cron_type)
{
	...
	case 'rpguser':
		...
		if (time() - $config['activity_last_time'] <= $config['activity_time']*4)
		{
			
			break;
		}
	
		include_once($phpbb_root_path . 'includes/cron/cron_user.' . $phpEx);
		
		if ($use_shutdown_function)
		{
			register_shutdown_function('tidy_rpguser');
		}
		else
		{
			tidy_rpguser();
	...
	break;
	case 'tidy_useractivity':
		
		if (time() - $user->data['activity_last_time'] <= $config['activity_time'])
		{
			break;
		}
		...
		if ($use_shutdown_function)
		{
			...
			register_shutdown_function('tidy_useractivity');
		}
		else
		{
